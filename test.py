...
def movie(event):
    ...
    moviename = re.sub('[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890]', '', movie).replace(' ', '')
    if moviename !='':
        cut = mmcut(movie)
        words = []
        for row in cut:
        ...
        with graph.as_default():
            ...
            pooled_outputs = []
            x = tf.placeholder(tf.float32, shape=[None, wordCount * weightColumn1]) #inputvec
            y_ = tf.placeholder(tf.float32, shape=[None, cateDimension])    #outputvec
